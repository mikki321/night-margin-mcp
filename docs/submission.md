...
Ten tools: three read-only analyses (`analyze_portfolio`, `compare_strategies`, `gap_night_check`), a four-step decision loop (`propose_decisions` drafts floor prices → `apply_decision` previews the exact RM API payload, then writes it only with `confirm: true` → `revert_decision` restores the pre-write snapshot), `check_alerts` for a daily cron, `review_history` — a read-only mirror of your own past months net of an estimated turnover cost — and `margin_copilot`, which ranks every below-floor cluster into a money-move, weighs the options for each (hold / guard the floor / raise the minimum stay) and marks the recommended one, staging any change through the same confirm-gated write path.
...
I run Lapland Host, a property management company with 73 short-term rental listings in Rovaniemi, Finnish Lapland. This tool wasn't built *about* property management; it was built *inside* one: the cost model comes from our own turnover operations, and every tool was tested live against our own Wheelhouse portfolio during the hackathon week — including the seasons where it correctly tells me to do nothing. Building it against a high-ADR, long-stay portfolio is what surfaced the honest boundary of when a margin guard matters and when it doesn't; an operator with €90 city apartments and two-night stays sits on the other side of that line, and gets the most from it.

## How it maps to the judging criteria

**Business Value.** The honest case starts with when this tool tells you to do *nothing*. My winter is high-ADR: bookings average €481/night and turnover is 3.9% of gross, so the tool runs my November–March window and says "no leak — every booking covers its own turnover cost." Correct answer: leave prices alone. Then the shoulder season inverts it — ADR drops to €93, turnover becomes 26% of gross, and one booking already sold below cost. That is the exact moment the tool earns its place: live this week, 206 unsold nights across 9 of my listings carry a Wheelhouse recommendation below their cost floor, and `propose_decisions` drafted 20 fixes I can apply and undo. The same mechanism runs year-round for the operator this is really built for — low ADR, short stays, high occupancy, where a €70 turnover is 25–40% of a two-night booking's revenue. Any operator reproduces the whole thing with their own Wheelhouse key and one number: what a turnover costs them.

**Completeness.** Not a mockup: published on npm, one-command install, first result in under 60 seconds with zero config (clearly-labeled synthetic data), fully functional with a real Wheelhouse key. Ten working tools, a 372-test suite, actionable error messages, and a public demo page rendered from real server output. On a live 73-listing portfolio the first call takes ~90 seconds — one request per listing, serial, inside Wheelhouse's 60 req/min limit — then results are cached for 10 minutes.

**Technical Execution.** A TypeScript MCP server (official SDK, stdio) integrating the Wheelhouse RM API — listings, per-listing reservations, per-night price recommendations, min-stay calendars, monthly KPIs, and custom-rate writes — with serial requests and backoff for the 60 req/min limit, live-tested against my own 73-listing portfolio. Turnover costs flow through a pluggable adapter (manual / CSV / HTTP API) with a per-booking attribution cascade reported in every response. The output states its own limits: price recommendations cover a rolling 30-night horizon, so nights beyond it are reported as unchecked rather than silently passed.
...
**Category Fit.** Built by a property manager who runs 73 listings in Lapland — for operators like us. Developed against and live-tested on my own portfolio during the hackathon week, solving a problem my team hits every week. `review_history` is the clearest proof of that: run on my own 9 months of Wheelhouse data, it shows turnover ate 5% of gross in my high-ADR December and 34% in the shoulder season — the same honest boundary, drawn from my own history, that tells an operator when this tool earns its place and when it should tell them to do nothing.
